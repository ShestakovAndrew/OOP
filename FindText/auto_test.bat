...
%PROGRAM% test_files\more_match_in_multi_string.txt test > "%TEMP%\more_match_in_multi_string_output.txt"
...
%PROGRAM% test_files\file_empty.txt dog > "%TEMP%\file_empty_output.txt"
if NOT ERRORLEVEL 1 goto err
...
if NOT ERRORLEVEL 1 goto err
fc.exe "%TEMP%\non_match_in_string_output.txt" test_files\non_match_in_string_result.txt
if ERRORLEVEL 1 goto err
...
if NOT ERRORLEVEL 1 goto err
fc.exe "%TEMP%\non_match_in_multi_string_output.txt" test_files\non_match_in_multi_string_result.txt
if ERRORLEVEL 1 goto err

rem ожидаем ненулевой код, если файла не существует.
%PROGRAM% non-existing-file-name.txt arg 
if NOT ERRORLEVEL 1 goto err
