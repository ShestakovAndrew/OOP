...
rem ожидаем нулевой код, если в файле нет текста.
	%PROGRAM% test_files\file_empty.txt "%TEMP%\file_empty_result.txt" "кошка" "собака" > "%TEMP%\file_empty_output.txt"
	if ERRORLEVEL 1 goto err
	fc.exe "%TEMP%\file_empty_output.txt" test_files\replace_successfully.txt
	if ERRORLEVEL 1 goto err
...
rem ожидаем ненулевой код, если файлы идентичны.
	%PROGRAM% test_files\file_empty.txt test_files\file_empty.txt "кошка" "собака" > "%TEMP%\files_identical_output.txt"
	if NOT ERRORLEVEL 1 goto err
	fc.exe "%TEMP%\files_identical_output.txt" test_files\files_identical.txt
	if ERRORLEVEL 1 goto err
